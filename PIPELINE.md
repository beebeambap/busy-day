...
## 7. Phase C-2 — 보이싱 spread + 드론 (완료)

로드맵 §2 두 번째 묶음. **기온 → 보이싱**, **흐림 → 드론**.

### 7.1 보이싱 spread (`compose/arrange.py: _spread_for(warmth)`)

같은 코드도 음역대 폭이 다르면 따뜻함 ↔ 차가움 인지가 바뀐다.

| warmth | spread | 효과 |
|---|---|---|
| < 0.30 (추움) | `tight` | 모두 같은 옥타브에 모여 있음 (좁고 단단함) |
| 0.30–0.70 | `default` | 표준 close-position |
| > 0.70 (따뜻) | `wide` | 루트 -1옥타브, 톱 +1옥타브 (개방감) |

`compose/scales.py: chord_pitches(spread=...)` 가 실제 변경 수행.
검증: D-ionian I = [50,54,57] tight → [38,54,69] wide.

### 7.2 드론 (`compose/arrange.py: drone_on = cloud_pct >= 70`)

흐린 날(cloud_pct ≥ 70%)에만 발생. 섹션마다 한 음씩, 토닉 -2옥타브
(MIDI 24 근처)에 매우 낮은 벨로시티(24~32)로 깔린다. 안개처럼
믹스 밑을 받쳐주는 역할.

- MIDI 채널 3, GM 88 (New Age Pad) 로 export → DAW 호환
- 브라우저: `makeDrone()` — 두 sine (C1/C2, 디튠 4cents) + lowpass 280Hz
  + 리버브 라우팅. -22dB 볼륨

검증:
| 케이스 | temp | cloud | spread | drone events |
|---|---|---|---|---|
| 추운 맑은 날 | -2°C | 10% | tight | 0 |
| 비 오는 흐린 날 | 12°C | 95% | default | 5 (섹션당 1개) |
...
| 1순위 환경음 | ✅ |
| 2순위 리버브 wet → 습도 | ✅ |
| 2순위 보이싱 spread → 기온 | ✅ Phase C-2 |
| 2순위 드론 → 흐림 | ✅ Phase C-2 |
| 2순위 노트 밀도 → 풍속 | 미착수 (Muji 톤 보호 위해 신중 설계 필요) |
| 2순위 휴머나이즈 → 기압 | 미착수 |
| 3순위 무드별 화성 풀 | 미착수 |
| 4순위 시간/상황 무드 | 미착수 |
| 5순위 마림바·하프·클라리넷 | 미착수 |

다음 사이클 후보 — Phase C-3: 노트 밀도(풍속) + 무드별 화성 진행 풀.

---

## 8. Phase C-3 — 무드별 화성 진행 풀 + 풍속→밀도 (완료)

### 8.1 무드별 진행 풀 (`compose/progressions.py`)

intent 가 설정된 곡은 마르코프 대신 6개 무드별로 큐레이션된 풀(각 4종)
중에서 시드 RNG로 하나를 골라 섹션 길이만큼 루프. cron 의 `auto`
곡은 기존 마르코프 그대로 — 매일 곡의 "낯익은 본진"은 유지.

| 무드 | 대표 진행 | 모드 |
|---|---|---|
| calm | I-iii-vi-IV (안정·순환) | (날씨가 결정) |
| warm | I-V-vi-IV (포근한 해결) | (날씨가 결정) |
| **wistful** | vi-IV-i-V (단조 변형) | **dorian 강제** |
| lively | I-IV-V-I (명확한 전진) | (날씨가 결정) |
| **after_rain** | **bVII-IV-I (믹솔리디안)** | **mixolydian 강제** |
| sleep | I-iii-IV-iii (흔들림·서스펜션) | (날씨가 결정) |

`after_rain` 의 모드 강제가 새로 추가됨 — bVII-IV-I 진행은 mixolydian
에서만 실제 ♭VII 로 들리므로(ionian 에선 vii° 디미니쉬드).

### 8.2 풍속 → 노트 밀도 (`arrange._apply_wind_density`)

`wind_factor = clip(wind_mps / 8, 0, 1)`

| 구간 | 동작 |
|---|---|
| < 0.30 | thin: 짧은 내부 음을 ~45% 확률로 제거하고 dur 을 앞 음에 합산 |
| 0.30–0.60 | 그대로 |
| > 0.60 | dense: 0.5박+ 음을 분할해 stepwise passing tone 삽입 (max 80%) |

**Muji 톤 보호**: 0.30~0.60 구간(2.4~4.8 m/s)이 가장 흔한 풍속이라
대부분 곡은 손대지 않음. 양 끝에서만 작은 변화를 줘서 "정적인 날은
더 비우고, 바람 부는 날은 살짝 채우는" 느낌만.

검증: 같은 날짜·같은 시드에서 wind=1 → 3.8/bar, wind=7 → 6.1/bar.
...
다음 사이클 후보 — Phase C-4: 휴머나이즈 → 기압 매핑 (남은 2순위 마지막) +
시간/상황 무드(작업 중·낮잠·산책 등; 4순위 — SEO 효과 큼).
